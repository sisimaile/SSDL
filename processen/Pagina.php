...
class Pagina extends Connection
{
    ...
    protected function getOnePagina($id){
    ...
    public function addPaginas($titel, $message, $image)
    {
        if(!empty($titel) && !empty($message)){
            ...
            if ($results > 0){
                header("Location: pagina.php-toevoegen.php?pagina.php-bestaat-al");
            }else{
                ...
                if ($link){
                    ...
                    header("Location: pagina.php-toevoegen.php?pagina-is-niet-aangemaakt");
                }
